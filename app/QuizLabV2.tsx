...
const questions: Question[] = [
  ...
  { id:42, tag:"SWEEP ANGLE", prompt:"已知 TLI 点地心位置 r₀ 和月球影响球穿越点地心位置 r₁，张角（sweep angle）Δθ 如何求解？", options:["令 cos Δθ = û_r0 · û_r1，直接由两端径向单位向量点积给出，无需迭代", "必须数值迭代求解 Kepler 方程", "Δθ 必定等于 180°（因为是对称椭圆）", "必须先求出偏心率 e₁ 才能反解 Δθ"], answer:0, why:"两端点的位置向量已完全确定：r₀ 由发射参数给出，r₁ = rm + r₂ 由到达角 λ 确定。两端单位位置向量的点积 cos Δθ = û_r0 · û_r1 直接闭式给出地心扫过角 Δθ。", trap:"Δθ 是两个端点在地心的几何夹角，不是真近点角 θ₀；真近点角必须从转移椭圆的近地点起算。" },
  ...
  { id:61, tag:"LOI CAPTURE", prompt:"一艘飞船从地球出发，沿双曲线轨道进入月球影响球（SOI）。在其到达近月点（r_p = 2759 km）处，航速为 2.070 km/s，而当地逃逸速度为 1.885 km/s。若飞船在近月点不执行任何脉冲点火机动，将发生什么物理后果？", options:["月球潮汐引力摩擦将自动使飞船圆化进入低月轨道", "飞船将永久被束缚在偏心绕月椭圆轨道中", "飞船必将沿着出射双曲线渐近线飞离月球影响球，重返地心空间", "飞船将在近月点立即撞击月表坠毁"], answer:2, why:"在月心二体引力场中机械能守恒。航速高于逃逸速度（2.070 > 1.885 km/s）意味着比能量严格为正（ε > 0，双曲线）。真空中无大气阻力，引力无法自发吞噬动能。“Capture is a choice, not an event”：除非主动执行逆向减速点火（LOI），否则飞船必将沿出射渐近线飞离月球！", trap:"选项 A 幻想月球引力会主动把飞船留住，但二体引力是保守场，绝不可能自发实现负能量闭合捕获；选项 D 混淆了半径与高度（近月点净高度为 1021.6 km）。" },
  { id:62, tag:"STAGED CAPTURE", prompt:"在分级环月入轨（Staged LOI）机动中，若在近月点精确执行一次切向逆行减速点火，新轨道的近月点半径与远月点半径将如何变化？", options:["近月点半径被压低，而远月点半径保持不变", "近月点半径严格保持不变，而远月点半径被大幅压低", "近月点与远月点半径同等幅度减小", "近月点半径抬高，远月点半径变成无穷大"], answer:1, why:"轨道机动的铁律：在轨道上任意一点点火，新轨道必然再次经过点火点！在近月点执行逆向点火时，点火点位置 r_p 瞬间不变，速度减小使其成为新椭圆的近地点（近月点），而半长轴 a 缩短导致远月点 r_a = 2a - r_p 大幅降低（从无穷大双曲线降至闭合椭圆）。", trap:"日常直觉以为‘刹车会让脚下的高度立刻下降’，但在轨道力学中，近月点刹车改变的是半圈之后的对侧远月点！" },
  { id:63, tag:"GRAVITY LOSS", prompt:"为了使登月舱在紧邻月表的低空对抗月球重力保持静止悬停，每悬停 1 秒钟需要持续消耗多少等效速度增量 Δv？", options:["9.807 m/s", "0.000 m/s（因为飞船在空间中静止不动，位移为零）", "1.624 m/s", "1.692 km/s"], answer:2, why:"在悬停时，发动机必须产生向上推力以精确抵消月球重力：T/m = g_m = 1.624 m/s²。根据牛顿定律，推力加速度积分即为推进剂消耗的有效速度增量：d(Δv)/dt = g_m = 1.624 m/s。悬停 60 秒将白白烧掉近 100 m/s 的宝贵燃料！", trap:"物理学中‘做功 W = F·s = 0’常让人误以为悬停不耗能，但火箭发动机依靠向外喷射工质产生反推力，推进剂质量流率严格正比于推力！" },
  { id:64, tag:"DOI DESCENT", prompt:"从 100 km 环月圆轨道出发，着陆器执行一次动力下降轨道切入（DOI）点火以将近月点降至 15 km（阿波罗与现代标准剖面）。该次脉冲点火所需的 Δv 大小约为：", options:["约 22 ~ 25 m/s", "约 1.633 km/s", "约 835 m/s", "约 3.150 km/s"], answer:0, why:"DOI 是一次微小的半霍曼转移：100 km 圆轨道航速 v_circ = √(μ/r1) = 1.633 km/s；而 100×15 km 转移椭圆远地点速度 v_t1 = 1.611 km/s。两者之差仅为 Δv = 1.633 - 1.611 = 0.022 km/s = 22 m/s！用极小代价即可换取半圈后 85 km 的自由落体高度下降。", trap:"千万别错选 1.633 km/s（那是消除全部环月轨道速度）或 835 m/s（那是月球捕获 LOI）。DOI 只是轻轻踩一脚刹车改变轨道形状！" },
  { id:65, tag:"DESCENT STEERING", prompt:"在阿波罗 11 号动力下降着陆过程中，休斯敦地面控制中心呼叫了‘剩余 30 秒燃油’。该呼叫在工程上的真正含义是：", options:["下降发动机油箱将在 30 秒内彻底抽干并熄火", "距离触发强制中止着陆（Abort Criteria）并点火上升级发动机逃逸只剩 30 秒决策时间", "登月舱必须在 30 秒内耗尽全部燃料以减轻自重", "飞船蓄电池电量将在 30 秒内耗尽"], answer:1, why:"飞行规则规定了严格的低油位红线（Bingo fuel / Abort criteria）：在触地前必须保留 20 秒左右的缓冲推进剂，用于一旦无法安全着陆时紧急切断下降级、点火上升级发动机返回环月轨道。‘30 秒呼叫’意味着离必须拍下中止按钮只剩 30 秒决策窗口，而非油箱彻底抽干！", trap:"大众媒体常渲染为‘只差 30 秒发动机就要熄火砸毁’，但在严密的载人航天工程中，这指的是中止与逃逸的最后决策窗口。" },
  { id:66, tag:"LUNAR GEOMETRY", prompt:"一艘飞船进入 100 km 环月极圆轨道（i = 90°）。月球在其轨道面下方自转。若月球恒星自转周期为 27.32 天，该卫星需要多长时间其星下点轨迹才能遍历月球的全部经度（全球覆盖）？", options:["恰好 27.32 天", "约 13.66 天（半个月球自转周期）", "约 1.97 小时（绕一圈轨道的时间）", "由于轨道是极轨，卫星永远只能看到同一条经线"], answer:1, why:"极轨卫星每圈轨道同时穿过月球的正面和背面（相隔 180° 经度）。当月球自转 180°（即 27.32 / 2 ≈ 13.66 天）时，降交点轨迹正好扫过此前升交点未覆盖的半球，从而在半个恒星月内实现全球所有经度的全覆盖！", trap:"容易误选 27.32 天（以为月球必须转整整一圈），但极轨卫星一圈能同时扫过两个相隔 180° 的经度段，因此只需转半圈（13.66 天）！" },
  { id:67, tag:"LUNAR GEOMETRY", prompt:"飞船需要在月球周围执行轨道面改变机动（Δi）。在 100 km 环月圆轨道（v = 1.633 km/s）直接变轨所需的 Δv，与在远月点为 16,000 km 的大椭圆远月点执行相同 Δi 相比，代价大约是后者的多少倍？", options:["1.0 倍（圆轨道与椭圆轨道代价相同）", "约 2 倍", "约 7 倍（因为 100 km 轨道航速约 1.63 km/s，而远月点航速仅约 0.23 km/s）", "在远月点变轨代价无限大"], answer:2, why:"平面改变脉冲公式为 Δv = 2 v sin(Δi/2)，代价严格正比于当前航速 v！在 100 km LLO，v = 1.633 km/s；而在 200×16000 km 椭圆远月点，根据活力公式航速降至 v_a ≈ 0.233 km/s。比值 1.633 / 0.233 ≈ 7.0 倍！这就是为什么面外机动必须移到远地点/远月点执行。", trap:"切勿混淆 Oberth 效应（切向点火加速/减速在近地点最省）与平面改变机动（法向点火转弯在远地点最省，速度越慢转弯越便宜）！" },
  { id:68, tag:"LUNAR GEOMETRY", prompt:"飞船从地球出发时轨道倾角受到发射场纬度刚性约束（cos i_E = cos φ_L · sin Az），但为什么在地月转移轨道到达月球影响球（SOI）时，却能够几乎‘免费’对准任意月球轨道倾角（包括 90° 极轨）？", options:["因为月球引力在飞船穿过 100 km 高度前完全不存在", "因为自由转移轨道面朝向取决于近地点注入（TLI）时的几何几何瞄准，只需微调飞入月球 SOI 的 B 平面碰撞参数即可", "因为月球自转轴与地球赤道严格垂直", "需要消耗超过 3000 m/s 的燃料在半途中进行大角度面外转向"], answer:1, why:"月球轨道面相对地球赤道存在 18.3°~28.6° 的夹角，且月球处于三维空间。飞船从地球发射后，通过选择发射时刻（调整交点升经度 Ω）以及微调 TLI 速度矢量和中途修正（TCM），可以精确改变飞船在月球 SOI 边界处的入射瞄准点（B 平面偏移）。微小的中途修正（几十 m/s）就能自由决定入轨是赤道轨、顺行轨还是极轨！", trap:"很多初学者以为进入极轨必须在月球附近强行打 90° 转向 burn（那需要 2.3 km/s！），实际上在出发前靠几何瞄准就能实现‘免费对准’。" },
  { id:69, tag:"DOI DESCENT", prompt:"在阿波罗计划工程经验报告（NASA TN D-6846）中，启动动力下降（PDI）的近月点高度被确立在 15 km（50,000 ft）。这一高度选择的核心工程权衡（Trade-off）是：", options:["月球引力在 15 km 以上降为零", "兼顾重力损失最小化与地形避障安全（防止撞击月表高山及导航误差超限）", "登月舱降落伞在此高度开伞效率最高", "地球测控雷达无法探测 15 km 以下的目标"], answer:1, why:"从燃料角度看，PDI 越低越好（若在 100 km 就开大油门对抗重力，重力损失将高达数百 m/s）；但月球表面地形起伏剧烈，南极环形山山脊高达 6–8 km，叠加着陆器轨道测定误差（导航散布椭圆），若近月点设在 5~8 km，极易发生可控撞地（CFIT）。15 km 是兼顾燃油经济性与地形避障净空的黄金折中点！", trap:"选项 C 是荒谬干扰项（月球真空根本没有降落伞）；选项 A 违背万有引力定律。" },
  { id:70, tag:"POWERED DESCENT", prompt:"在 15 km 近月点（PDI）处，登月舱入轨航速为 1.692 km/s。若比冲 Isp = 310 s（标准两组元可贮存推进剂），仅消除这一水平速度动能（暂不计重力损失），理想火箭方程所需的初始与最终质量比 m0/mf 约为：", options:["1.00", "1.25", "1.76", "4.50"], answer:2, why:"代入齐奥尔科夫斯基火箭方程：有效排气速度 c = Isp · g0 = 310 × 9.807 ≈ 3040 m/s = 3.040 km/s。质量比 m0/mf = exp(Δv / c) = exp(1.692 / 3.040) = exp(0.5566) ≈ 1.745 ~ 1.76！这意味着起跳时至少有 43% 的质量纯粹是用来刹停水平速度的推进剂。", trap:"切勿忘记公式中指数函数的增长，且计算时 Δv 与 g0 的单位必须严格统一（km/s 与 m/s 的换算）。" },
  { id:71, tag:"LOI CAPTURE", prompt:"将飞船从到达近月点（r_p = 2759 km, v_p = 2.070 km/s）送入目标圆轨道。对比两种方案：方案 A 直接捕获至 1021 km 圆轨道（Δv_A = 0.736 km/s）；方案 B 先在近月点刹车进入 1021×3141 km 椭圆，再在远月点进行圆化。方案 B 在推进剂消耗与工程操作上的最大优势是：", options:["方案 B 完全不需要发动机点火", "方案 B 分散了点火热载荷，且可在高远月点以极低代价执行轨道面修正（nodal trim），避免直接捕获的单点失败风险", "方案 B 的总速度增量比方案 A 增加 5000 m/s", "方案 B 可以在月球大气中利用降落伞减速"], answer:1, why:"直接捕获（Route A）要求大推力发动机在几分钟内一次性完成全部减速，热冲击大、一旦点火提前关机飞船将直接飞离月球。分级捕获（Route B）将点火拆分为两次，初次点火只需进入闭合椭圆即可确保被月球束缚，而且在速度极低的远月点进行倾角和节点修正代价极低！", trap:"不要以为单次点火总能省油，分级入轨是阿波罗、嫦娥和阿尔忒弥斯标准的安全基准工程实践。" },
  { id:72, tag:"LUNAR GEOMETRY", prompt:"理论上极轨道（i = 90°）的一阶交点退行率因 cos(90°) = 0 而为零。然而 NASA 月球勘测轨道飞行器（LRO，50 km 极轨）在实际运行中，其轨道交点为何仍以每年数度的速率持续漂移？", options:["LRO 发动机发生推进剂持续泄漏", "月球高阶非球形引力摄动项（如 C22 等非带谐项）与地球/太阳三体引力打破了纯 J2 模型的对称性", "月球自转轴每十年翻转一次", "爱因斯坦广义相对论时空弯曲产生的巨大拖拽"], answer:1, why:"教科书纯 J2 摄动公式只考虑了中心天体的轴对称扁率。但月球是一个三轴不规则椭球体（triaxial body），其非带谐项（特别是 C22、质量瘤引力异常）以及地球和太阳的第三体引力摄动不可忽略，这些摄动项会持续驱动极轨平面的进动与漂移！", trap:"不要死背‘极轨退行率恒等于零’——那仅适用于理想无外力摄动的纯 J2 扁球体模型！" },
  { id:73, tag:"GRAVITY LOSS", prompt:"从 15 km 近月点动力下降过程中，阿波罗级着陆器消耗了 1.781 km/s 的实测 Δv，相比于纯动能消除基准（1.692 km/s）额外多出了约 89 m/s。这多出的 89 m/s 速度增量在物理上属于：", options:["完全是月球大气气动阻力损失", "推力矢量向上偏转抵抗月球重力所累积的重力损失（Gravity Losses）及姿态转向损失", "发动机喷管制造公差导致的推力测量误差", "月球磁场对金属船体的电磁阻尼消耗"], answer:1, why:"动力下降期间，推力不能始终纯水平逆向喷射（否则飞船会在重力作用下失控撞地），必须向上抬起一定俯仰角以平衡月球引力：Δv_grav = ∫ g_m sin(θ) dt。这段对抗引力所白白耗费的推进剂即为重力损失，再加上姿态转向损耗，使得总 Δv 达到约 1.78~1.85 km/s！", trap:"月球是超高真空（无大气），根本不存在任何气动阻力损失！" },
  { id:74, tag:"DESCENT STEERING", prompt:"月球动力下降二维运动方程中采用的线性正切导引律（Linear Tangent Steering Law: tan θ(t) = a · t + b），其数学与工程本质是：", options:["着陆器像电梯一样垂直笔直坠落", "在均匀引力场与恒定推力假设下，由庞特里亚金极大值原理推导出的推进剂最优俯仰角控制解析解", "控制算法死机时的紧急备用随机摆动模式", "由于传感器故障导致无法测量俯仰角时的估算"], answer:1, why:"在最优控制理论中，应用庞特里亚金极大值原理（Pontryagin's Maximum Principle）求解推力矢量轨迹优化问题，协态变量在常重力场下随时间线性演化，推导得出推力方向角的正切值严格随时间呈线性函数：tan θ(t) = a·t + b。它既保证了燃油消耗最优，又具备闭式解析解利于机载计算机实时计算！", trap:"线性正切导引是航天制导史上的经典杰作（广泛用于土星五号入轨与登月下降），并非经验拟合或备用妥协方案。" },
  { id:75, tag:"DESCENT STEERING", prompt:"阿波罗登月剖面分为三段：制动段（15 km 到 2.5 km）、进近段（2.5 km 到 150 m）、终端垂直下降段（150 m 到接触）。NASA 将进近段飞船仰角显著立起的关键目的是：", options:["增加大气气动阻力以实现免费减速", "将舷窗朝向月表着陆区，使宇航员看清地形坑洼并评估是否需要手动干预重选着陆点（LPD）", "让太阳能帆板对准太阳充电", "防止下视雷达因仰角过大而丢失信号"], answer:1, why:"在制动段，登月舱几乎平躺（发动机向前喷气减速），航天员只能看到黑色星空。在 2.5 km 进入进近段后，导引系统指令飞船大幅抬头立起（仰角转为 45°~60°），舷窗朝向前下方，航天员终于能透过三角窗看清着陆区的环形山和岩石，并通过 Landing Point Designator (LPD) 评估是否需要手动操纵杆改降平坦区域！", trap:"进近段抬头会稍微增加重力损失，但为了保障宇航员的肉眼着陆视野和安全重选着陆点，这一工程代价是绝对必需的。" },
  { id:76, tag:"LUNAR GEOMETRY", prompt:"一艘飞船进入 100 km 环月圆轨道（v = 1.633 km/s），由于导航偏差导致入轨倾角与预定目标存在 Δi = 20° 偏差。若在 100 km 圆轨道执行单次脉冲变轨修正，所需 Δv 约为多少？", options:["约 0.022 km/s", "约 0.284 km/s", "约 0.567 km/s", "约 1.633 km/s"], answer:2, why:"圆轨道平面改变公式：Δv = 2 v sin(Δi / 2)。代入数值：Δi/2 = 10°，sin(10°) ≈ 0.17365；Δv = 2 × 1.633 × 0.17365 ≈ 0.567 km/s (567 m/s)！仅 20° 的平面修正就吃掉了半次捕获点火的推进剂，再次证明在低轨改变倾角代价极其惨烈！", trap:"学生容易误算成 2 v sin(Δi) 或忘记除以 2，或者错选 0.284 km/s（少乘了系数 2）。" },
  { id:77, tag:"LOI CAPTURE", prompt:"在分级环月捕获中，将捕获椭圆的远月点推得更高（例如从 3,000 km 提高到 16,000 km），首次点火 LOI-1 所需的 Δv 会显著减小。然而工程上为何不能无限抬高远月点？", options:["飞船超过 20,000 km 会在月球大气中烧毁", "当远月点接近月球影响球边界（SOI ≈ 66,100 km）时，地球第三体引力摄动将使轨道极度不稳定甚至直接将飞船拽离月球，且轨道周期过长大幅推迟着陆窗口", "远月点越高，微波通信越无法穿透太阳风", "阿波罗飞船发动机会在 10,000 km 以上结冰失效"], answer:1, why:"抬高远月点确实能省推进剂，但有两个致命限制：① 当远月点接近月球影响球半径（r_SOI ≈ 66,100 km）时，地球的第三体引力摄动急剧增强，极易把飞船拉出月球轨道；② 椭圆轨道半长轴增大导致轨道周期暴增（从几小时变成几天甚至一周），严重耽误着陆场的晨昏光照窗口！", trap:"不要以为引力势能越高越好，月球引力场的作用范围受到地球强大潮汐力的严厉截断。" },
  { id:78, tag:"SOUTH POLE", prompt:"现代登月任务（如月船三号、SLIM、阿尔忒弥斯）普遍将目标锚定在月球南极。然而月球南极在着陆下降阶段给导航与工程带来的最严峻挑战是：", options:["月球南极地平线极低高度的太阳入射角导致超长阴影与极端对比度，且地球常年贴近地平线极易遮挡测控视线", "南极存在强烈的地磁暴与范艾伦辐射带", "南极表面被深达数千米的液态水海洋覆盖", "月球南极的重力加速度是赤道的十倍以上"], answer:0, why:"由于月球自转轴倾角仅 1.54°，在南极太阳光几乎贴着地平线水平照射，造成极度拉长的漆黑阴影和刺眼的岩壁反光，光学地形相对导航（TRN）极易失效；同时，地球在南极天空永远贴近地平线低空晃动，稍微降落在环形山内侧就会彻底失去对地通信！", trap:"月球没有液态水（只有永久阴影区的深层水冰），也没有全球磁场或辐射带，重力加速度全球几乎一致（1.62 m/s²）。" },
  { id:79, tag:"LANDER FORENSICS", prompt:"印度月船二号（Chandrayaan-2）登月器与商业着陆器 ispace 白兔号（Hakuto-R）失利的飞行事故调查表明，现代无人登月动力下降最脆弱的薄弱环节在于：", options:["太阳能电池板在太空中无法发电", "传感器异常处理与制导软件容错逻辑（高度突变被算法错误过滤、或控制力矩饱和发散）", "火箭发动机被外太空陨石击穿", "火箭燃料在极寒环境下彻底冻结"], answer:1, why:"事故调查报告证实：月船二号是因为姿态修正推力算法发散导致失控；白兔号（Hakuto-R）则是因为飞跃环形山陡峭边缘时，激光测距仪测到的真实高度骤变被软件里的‘异常值滤波器’强制剔除，导致飞船误以为自己还在几公里高空，提前悬停耗尽燃料下坠。现代登月的最大考验在于严密鲁棒的制导软件容错设计！", trap:"硬件推进剂通常配有加热器，现代无人登月的致命破局点往往隐藏在自治导航与传感器融合（GNC）软件的边缘工况逻辑中。" },
  { id:80, tag:"MISSION BUDGET", prompt:"在第八讲梳理的全任务推进剂预算链中：LEO出发TLI（~3150 m/s）+ 环月捕获LOI（~850 m/s）+ 下降注入DOI（~25 m/s）+ 动力着陆PDI及重力损失（~1850 m/s），总Δv需求高达 6000 m/s。若采用传统双组元化学推进（Isp = 310 s），单级着陆器推进剂占总质量的比例（推进剂质量分数）至少需要达到：", options:["约 15%", "约 50%", "约 86%（推进剂占绝大部分，结构与有效载荷仅剩约 14%）", "理论上单级无法实现，推进剂比例必须超过 100%"], answer:2, why:"根据齐奥尔科夫斯基火箭方程：m0/mf = exp(Δv / [Isp · g0]) = exp(6000 / [310 × 9.807]) = exp(1.973) ≈ 7.19。推进剂质量分数 m_prop / m0 = 1 - 1/7.19 ≈ 0.861（86.1%）！这意味着结构、航电、热控、着陆架以及全部科学载荷合计只能占初始湿重的 13.9%。这就是为什么阿波罗必须采用‘土星五号多级火箭 + 独立登月舱（LOR）’分级架构的根本力学原因！", trap:"很多初学者低估了指数增长的威力。6000 m/s 在 310 s 比冲下对单级结构系数提出了近乎物理极限的苛刻要求！" },
];
...
const lessons: Lesson[] = [
  ...
  { no:24, phase:"WEEK 7 · L7", title:"环月捕获与分级入轨：LOI、Oberth 效应与倾角代价", goal:"掌握月心双曲线捕获本质、分级捕获（椭圆过渡）省燃料机理、月球 Oberth 效应与面外变轨代价。", plain:[
    "飞船从地球进入月球影响球（SOI）时处于双曲线轨道，比能量严格为正（ε > 0，通常 v_p ≈ 2.07 km/s > v_esc ≈ 1.885 km/s）。如果不点火刹车，引力只能让飞船弯曲偏转后原样飞离宇宙深空（Capture is a choice, not an event）。近月点逆向点火（LOI）是唯一的入轨手段。",
    "直接捕获到 100 km 低月圆轨道（LLO）需要极大的 Δv（约 835 m/s），所有推进剂必须在数分钟内一次性烧完，发动机热负荷大且缺少调整余量。阿波罗与现代任务普遍采用分级捕获（Staged LOI）：LOI-1 先将飞船刹车至高偏心率捕获椭圆（如 100×3141 km），随后在极其缓慢的远月点以极小的代价微调轨道面（nodal trim），最后再于近月点执行 LOI-2 圆化。",
    "月球上同样遵循 Oberth 效应：机械能变化量 ΔE = v · Δv，在近月点（v 最高处）刹车获得的动能消除效率最高！而在低月轨道改变轨道倾角（plane change）代价堪称灾难：100 km LLO 速度达 1.63 km/s，转弯 90° 需 Δv = 2 v sin(45°) ≈ 2.3 km/s！因此必须在地月转移段就直接对准目标轨道面（free transfer plane targeting）。",
  ],
  formulas:[
    {"eq": "Δv_LOI = v_p - v_circ = v_p - √(μ_m / r_p)", "note": "直接捕获至近月点圆轨道的脉冲速度增量。"},
    {"eq": "ΔE = v · Δv + ½ (Δv)² (Oberth Effect)", "note": "速度 v 越大，相同 Δv 改变的比轨道能量越多。"},
    {"eq": "Δv_plane = 2 v sin(Δi / 2)", "note": "圆轨道平面改变代价；在 100 km LLO（v=1.63 km/s）极度昂贵。"},
  ],
  terms:[
    ["LOI", "月球轨道捕获（Lunar Orbit Insertion）"],
    ["Staged Capture", "分级捕获：先入大椭圆（LOI-1）再圆化（LOI-2）"],
    ["Free Plane Targeting", "转移面自然对准：发射时直接瞄准月球自转轴倾斜面，避免入轨后变轨"],
    ["Apollo 11 Profile", "阿波罗11号分级入轨：LOI-1 入 111×314 km，LOI-2 调至 101×122 km"],
  ],
  example:"阿波罗 11 号 LOI-1 点火时长约 357 秒，在月球背面近月点将双曲线航速削减至 111×314 km 椭圆；两圈轨道检查无误后，LOI-2 点火 17 秒圆化至 101×122 km，充分利用了分级捕获的安全与热裕度！", check:{"q": "为什么不能在月球 SOI 边界（r ≈ 66,100 km）刚进入时就执行捕获减速？", "a": "在 SOI 边界航天器速度极低（接近 v_inf ≈ 0.8–1.0 km/s），缺乏 Oberth 效应放大。在近月点（v ≈ 2.07 km/s）减速能以最小的 Δv 换取最大的比轨道能下降！"} },
  { no:25, phase:"WEEK 7 · L7", title:"到达几何：自由转移对准、等待期与 Route A vs B", goal:"理解地月交会几何选择（Route A vs Route B）、月球等待期（loiter phase）与星下点漂移对着陆窗口的影响。", plain:[
    "到达月球时，飞船的近月点位置和轨道面朝向取决于地球出发时的轨道几何。通常存在两条路径：Route A（在月球公转轨道外侧掠过，先到达远月点再降至近月点）与 Route B（在月球公转内侧或 leading/trailing side 直接掠过）。Route A 能够提供更高的自由返回安全性，但通常耗时更长或需要微调入轨角。",
    "由于月球自转极为缓慢（自转周期等于公转恒星月 27.32 天，月表赤道自转线速度仅约 4.6 m/s），极轨卫星在轨道面上每绕一圈（约 2 小时），月表在下方仅西移约 0.55°（约 17 km）。这意味着极轨卫星需要约 13.6 天才能扫描完半个甚至整个月球表面！",
    "在正式着陆前，飞船往往需要在停泊轨道进行‘等待期（Loiter phase）’。这不仅是为了检查着陆器系统和导航滤波收敛，更是为了等待着陆区的太阳光照达到最佳着陆仰角（如 5°~15°，长阴影凸显岩石坑洼）以及等待轨道面漂移（nodal regression）精确掠过着陆场正上方。",
  ],
  formulas:[
    {"eq": "T_rev ≈ 2π √(a³ / μ_m) ≈ 118 min (for 100 km LLO)", "note": "低月轨道运行周期约为 2 小时。"},
    {"eq": "Δλ_groundtrack = (2π / 27.32 d) · (118 min) ≈ 0.54°", "note": "每圈轨道月球表面相对星下点的自转偏移张角。"},
  ],
  terms:[
    ["Lunar Loiter", "环月等待期：在停泊轨道等待光照条件与星下点对准"],
    ["Route A / Route B", "地月到达两类几何构型（外侧绕入 vs 内侧直接到达）"],
    ["Ground Track Coverage", "极轨星下点覆盖：因月球自转极慢，全球扫描需两周"],
  ],
  example:"阿波罗任务选择在月球清晨着陆（太阳高度角 7°–12°），长长的环形山阴影能让阿姆斯特朗看清巨石和深坑；因此飞船必须在 100 km 圆轨道等待（loiter）数圈，精确匹配着陆点晨昏线！", check:{"q": "为什么月表低自转速度（4.6 m/s）对极轨卫星星下点覆盖有利也有弊？", "a": "利在于单轨重叠度高，高分辨率成像极其连续稳定；弊在于星下点漂移极慢，要遍历全球或重返某个特定经度需要等待很多天。"} },
  { no:26, phase:"WEEK 7 · L8", title:"着陆准备：动力下降注入（DOI）与 15 km 极限权衡", goal:"理解下降轨道注入（DOI）的半霍曼变轨机理、为什么近月点选在 15 km 以及月表地形安全红线。", plain:[
    "从 100 km 环月圆轨道不能直接垂直刹车砸向月表——重力加速度会疯狂吞噬推进剂！正确的做法是执行动力下降注入（Descent Orbit Insertion, DOI）：在着陆点反面（半圈前，Δθ = 180°）轻踩刹车（逆向点火约 22~25 m/s），将轨道变成 100 km × 15 km 的霍曼转移椭圆。",
    "经过半圈自由滑行（耗时约 58 分钟，不需要耗费任何燃料），飞船自然下落至着陆点上方的 15 km 近月点，到达动力下降起点（PDI）。",
    "为什么近月点必须选在 15 km 左右？这是一个生与死的工程权衡（Trade-off）：如果近月点过高（如 50 km），主减速必须在稀薄的高空开始，重力损失将暴增数百 m/s；如果近月点过低（如 5 km 或 8 km），月球南极高达 6–8 km 的撞击坑边缘山脉（如 Malapert 峰）和导航误差椭圆将直接导致飞船在点火前撞山（CFIT 风险）！15 km 是兼顾地形安全与燃油最优的黄金分水岭。",
  ],
  formulas:[
    {"eq": "v_circ = √(μ_m / r_1) ≈ 1.633 km/s (r_1 = 1837.4 km)", "note": "100 km 圆轨道巡航速度。"},
    {"eq": "v_t1 = √(μ_m [2/r_1 - 2/(r_1 + r_p)]) ≈ 1.611 km/s", "note": "100×15 km 转移椭圆远月点速度。"},
    {"eq": "Δv_DOI = v_circ - v_t1 ≈ 22.0 m/s", "note": "阿波罗 LM / 现代着陆器经典的 DOI 速度增量！"},
  ],
  terms:[
    ["DOI", "下降轨道注入（Descent Orbit Insertion）：将近月点降至 15 km"],
    ["PDI", "动力下降点火（Powered Descent Initiation）：在 15 km 近月点开大油门减速"],
    ["Terrain Clearance", "净空裕度：避开月球南极 6–8 km 高度环形山山脊"],
  ],
  example:"阿波罗 11 号和 12 号由登月舱单独执行 DOI（约 23 m/s）；从阿波罗 14 号起，为了给登月舱省下宝贵悬停燃料，改由服务舱（CSM）在分离前统一执行 DOI，将整套飞船推入 100×15 km 轨道，登月舱直接在 15 km 分离点火！", check:{"q": "从 100 km 圆轨道执行 DOI 降至 15 km，所需的 Δv 大约是多少？", "a": "仅需约 22~25 m/s！用极微小的代价即可换取 85 km 的自由落体高度下降。"} },
  { no:27, phase:"WEEK 7 · L8", title:"动力下降主段：重力损失、线性正切导引与悬停代价", goal:"掌握速度消除基准（1.692 km/s）、重力损失积分机理、Apollo 三段式导引与悬停燃油惩罚（1.624 m/s²）。", plain:[
    "在 15 km 近月点（PDI），飞船的水平飞行速度高达 1.692 km/s！月表完全没有大气层，无法利用降落伞或气动减速，必须纯靠火箭反推将这 1.692 km/s 完全刹停到 0。根据齐奥尔科夫斯基火箭方程（以 Isp = 310 s 计），仅消除这部分动能就需要质量比 m0/mf = exp(1692 / 3041) ≈ 1.76（即起跳前推进剂占满载的 43% 以上）。",
    "更致命的是重力损失（Gravity Loss）：Δv_grav = ∫ g_m · sin(θ) dt。当飞船推力方向偏向垂直以抵抗月球重力时，每一秒钟月球引力都在白白偷走速度！此外，推力矢量的偏转还会带来转向损失（Steering Loss）。为了在非线性重力场中以最小燃料实现精确终端状态收敛，阿波罗和现代飞船广泛采用‘线性正切导引律’（Linear Tangent Steering Law: tan θ(t) = a · t + b）。",
    "阿波罗登月剖面分为三段：① 制动减速段（Braking phase，15 km 至 2.5 km，大推力逆向消除 95% 速度）；② 进近接近段（Approach phase，2.5 km 至 150 m，飞船立起仰角，让宇航员看清着陆区）；③ 最终垂直下降与悬停段（Terminal / Hover phase，150 m 到接触）。在月球表面悬停极其昂贵：每悬停 1 秒，就相当于白白付出 1.624 m/s 的 Δv！悬停 60 秒将烧掉近 100 m/s 推进剂，极易导致油尽坠毁。",
  ],
  formulas:[
    {"eq": "m_0 / m_f = exp(v_pdi / [Isp · g_0]) = exp(1692 / 3041) ≈ 1.76", "note": "速度消除的理想火箭方程质量比下限。"},
    {"eq": "Δv_gravity = ∫_0^t_burn g_m sin(θ_pitch) dt", "note": "重力损失积分：推力倾角越大、点火时间越长，损失越大。"},
    {"eq": "tan θ(t) = a · t + b", "note": "线性正切导引律（Linear Tangent Steering Law）。"},
    {"eq": "d(Δv) / dt_hover = g_m = 1.624 m/s²", "note": "悬停燃油惩罚：悬停 1 秒损失 1.624 m/s Δv。"},
  ],
  terms:[
    ["Speed Cancellation", "速度消除：消除 15 km 近月点 1.692 km/s 轨道动能"],
    ["Gravity Loss", "重力损失：抗衡月球重力所白白耗费的 Δv（通常 150–250 m/s）"],
    ["Linear Tangent Steering", "线性正切导引：倾角正切随时间线性变化的解析最优导引律"],
    ["Hover Penalty", "悬停代价：月球引力 1.624 m/s² 带来的持续推进剂流失"],
  ],
  example:"阿波罗 11 号在 150 米最后阶段发现原定着陆点布满巨石，阿姆斯特朗被迫手动切换悬停平移，额外消耗了 40 多秒燃料，着陆时推进剂仅剩不到 25 秒安全余量！", check:{"q": "月球表面悬停 30 秒需要付出多大推进剂等效的 Δv 惩罚？", "a": "Δv = 1.624 m/s² × 30 s ≈ 48.7 m/s！接近半次 DOI 变轨的代价。"} },
  { no:28, phase:"WEEK 7 · L8", title:"现代登月案例与全任务推进剂链条闭合", goal:"掌握现代着陆器动力架构与失利案例（SLIM 侧翻、月船3号升级、白兔号失效）、全任务 Δv 预算链与结构指数限制。", plain:[
    "近五年无人登月呈现出鲜明的新架构与实战经验：\n① 印度月船三号（Chandrayaan-3）：吸取月船二号因姿态控制发散坠毁教训，取消中央发动机，改用四台可节流发动机对角对称布局，软件容错由‘成功导向’改为‘失效安全’，并在 2023 年成功平稳着陆南极！\n② 日本 SLIM（精明着陆器）：演示 100 米级基于地形匹配（TRN）的针尖精确着陆，但因一台主发动机喷管在 50 米高度破裂脱落丧失偏航推力，最终倒立倾覆接触月表；\n③ 商业着陆器（ispace 白兔号、Peregrine、Nova-C）：白兔号因激光测距雷达穿越高环形山边缘时软件将悬崖跳变过滤为异常值，导致在高空误判着陆而耗尽燃料下坠。",
    "月球着陆任务的推进剂全任务预算链极其严酷（End-to-end budget chain）：\n从 LEO 出发需要：TLI (~3150 m/s) + 中途修正 TCM (~50 m/s) + LOI (~850 m/s) + DOI (~25 m/s) + 动力着陆 PDI 及重力损失 (~1850–2000 m/s) + 裕度 (~100 m/s)。总 Δv 需求高达 6000 m/s！",
    "根据火箭方程，如果单级火箭发动机 Isp = 310 s，质量比需求为 m0/mf = exp(6000 / 3041) ≈ 7.2。这意味着推进剂占总质量的 86.1%，结构、航电和有效载荷合计只能占 13.9%！任何系统增重都会在火箭方程指数放大下导致整个任务无法闭合。",
  ],
  formulas:[
    {"eq": "Δv_total = Δv_TLI + Δv_TCM + Δv_LOI + Δv_DOI + Δv_PDI + Δv_margin ≈ 6000 m/s", "note": "从 LEO 到月球表面软着陆的全任务速度增量预算。"},
    {"eq": "m_0 / m_f = exp(Δv_total / [Isp · g_0]) ≈ 7.2 (at Isp=310s)", "note": "严苛的单级质量比要求；推动了分级登月与阿波罗 LOR 架构的诞生。"},
  ],
  terms:[
    ["TRN", "地形相对导航（Terrain Relative Navigation）：实时匹配月表撞击坑光学图"],
    ["Structural Index", "结构系数 ε = m_struct / (m_struct + m_prop)，决定推进剂能否装下"],
    ["Fail-safe Logic", "失效安全逻辑：月船三号在极端分散下依然允许安全着陆的设计哲学"],
  ],
  example:"澳大利亚 ELO2 团队研发的 Roo-ver 半自主月球车计划于 2030 年由 NASA CLPS 商业着陆器投送至南极，面临长阴影、极寒水冰冷阱和极低通信仰角的严峻挑战！", check:{"q": "商业着陆器 ispace 白兔号（Hakuto-R）2023 年坠毁的根本原因是什么？", "a": "软件设定了高度跳变过滤阈值，当飞船飞越极深撞击坑边缘时，激光测距仪测到的真实高度突变被算法当作传感器故障过滤掉，导致飞船在 5 km 高空提前悬停耗尽燃油坠毁。"} },
];
...
const memoryCards = [
  ...
  { no:"16", title:"Departure & h₁", cue:"到达锚定坐标系，Eq 9.18 闭式解", body:"x 轴指向到达时的月球；扫过角 cos Δθ = û_r0 · û_r1 直接由点积求出；代入 Eq 9.18 联立消去真近点角，一步解出角动量 h₁。滑行 ~66.5h 月球走 ~36.3°（24.4 万 km），必须瞄准未来！", key:"题干出现 TLI geometry, sweep angle, Curtis 9.18 → 几何点积 + 闭式 h₁。" },
...
const formulas = [
  ...
  { name:"Sweep angle", eq:"cos Δθ = û_r0 · û_r1", use:"两端单位位置向量点积直接求地心张角", vars:"由 TLI 位置与月球到达角 λ 唯一确定" },
...
export default function QuizLab() {
  ...
  function newOnly(){ start("practice",questions.filter(q=>q.id>60)); }
  ...
  return (
    <main>
      ...
      <section className="hero" id="top">
        <div className="eyebrow"><span /> ZERO-BASE LEARNING PATH · LECTURES 1–8 (WEEKS 1–7) · WEEK 7-8 QUIZ</div>
        <h1>从零学会，<br/><em>再去做 20 题。</em></h1>
        <p className="hero-copy">不假设你记得 Space 1。从“位置、速度和轨道是什么”开始，经轨道机动、平面改变、地月转移，一路讲到月球捕获（LOI）、动力下降（DOI/PDI）、重力损失与南极登月实战。题库已扩充至 80 题，模拟考试随机抽 20 题。</p>
      ...
      <section className="scope-strip"><strong>Wk7-8 Quiz 覆盖：</strong><span>Wk1–3 · 机动基础</span><span>Wk3 L4 · 平面改变 + 发射 + 螺旋</span><span>Wk4–5 · 地月转移 + 影响球 + 自由返回</span><span>Wk6 · 3D 几何 + 三体积分 + 月球轨道</span><span>Wk7 · 环月捕获 + Oberth + 动力下降 + 现代登月</span><small>覆盖至最新 Week 7 全部讲义与课堂录音</small></section>
      ...
      <section className="decision section">
        ...
        <div className="decision-grid">
          ...
          <div><span>13</span><p>涉及 <b>TLI 几何或张角 Δθ</b></p><strong>→ cos Δθ = û_r0 · û_r1 + Curtis 9.18 闭式 h₁</strong></div>
          ...
          <div><span>22</span><p><b>环月捕获不踩刹车</b></p><strong>→ ε &gt; 0 双曲线，不刹车必沿出射渐近线飞离深空</strong></div>
          <div><span>23</span><p><b>动力下降近月点选 15 km</b></p><strong>→ 权衡重力损失最小化与南极 6–8 km 山脊净空避障</strong></div>
          <div><span>24</span><p><b>月表悬停燃油惩罚</b></p><strong>→ 每秒白烧 1.624 m/s Δv；悬停 60 秒耗掉近 100 m/s</strong></div>
          <div><span>25</span><p><b>LEO 至月表全任务预算</b></p><strong>→ 总 Δv ≈ 6000 m/s，单级推进剂占比 86%，迫使分级</strong></div>
        </div>
      </section>
      ...
      <section className="quiz section" id="quiz">
        {phase==="idle" && <div className="quiz-launch"><p>FINAL STEP / QUIZ ENGINE · {questions.length} QUESTIONS</p><h2>现在把理解变成分数</h2><div className="mode-grid"><button onClick={()=>start("practice")}><span>TRAIN</span><b>训练模式 · 全部 {questions.length} 题</b><small>逐题即时解析 · 不计时</small></button><button onClick={()=>newOnly()}><span>NEW</span><b>只练新考点 · Week 7 专属 20 题</b><small>环月捕获LOI、动力下降DOI/PDI与登月实战</small></button><button onClick={()=>start("exam")}><span>SIMULATE</span><b>30 分钟模拟</b><small>80 题随机抽 20 · 提交后统一解析</small></button></div><p className="honor-note">正式 quiz 为 closed book；本页只用于考前练习，考试时请关闭。</p></div>}
